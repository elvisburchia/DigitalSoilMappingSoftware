...
# Estimate execution time of the script
# UPDATE 02.02.2021
...
for (i in 1:nrpredictareas){
  for (j in 1:lenvariables){
    ...
    if (i==1){
      ...
      dir.create("Overview_Summary_Graphics")
    ...
    for (k in 1:lentransformations){
      ...
      write.csv(dff,file.path(fh,paste0("Explorative_Statistic_",var,"_",predictareas_name[i],".csv"))) 
      write.xlsx(dff,file.path(fh,paste0("Explorative_Statistic_",var,"_",predictareas_name[i],".xlsx")))
      d <- head(dff) 
      table <- tableGrob(d)
      title <- textGrob(paste0("Explorative_Statistic_",var,"_",predictareas_name[i]),gp=gpar(fontsize=15)) 
      footnote <- textGrob("", x=0, hjust=0, gp=gpar(fontface="italic")) 
      padding <- unit(2,"line") 
      table <- gtable_add_rows(table, heights = grobHeight(title) + padding, pos = 0) 
      table <- gtable_add_rows(table, heights = grobHeight(footnote)+ padding) 
      table <- gtable_add_grob(table, list(title, footnote), t=c(1, nrow(table)), l=c(1,2), r=ncol(table))
      png(paste0("Explorative_Statistic_",var,"_",predictareas_name[i],".png"), height = 200, width = 790)
      grid.newpage() 
      grid.draw(table) 
      dev.off()
    }
...
end_time <- Sys.time()
timeforsummarygraphics <- end_time-start_time
  ...
  for (i in 1:nrpredictareas){  #(i in 1:nrpredictareas)
    ...
    if (i==1){
      ...
      if (predictors_combination=="personalized") {
        ...
        nrofcombinations<-1
        predictors_combi_1<-c(1:length(predictors))
    ...
    for (k in 1:lange){ #(k in 1:lange)
      ...
      for (s in 1:1) { # (s in 1:3)
        ...
        for (l in 1:1) { #(l in 1:nrofcombinations)
          #start_time <- Sys.time()
          print(l)
          ...
          if (length(cutoffs)>=3) {
            ...
            if (s==1) {
              ...
              if (ai[k]=="raw") {
                ...
                nrmseRf <- nrmse(sim = obsValidRf[,varexplained], obs = obsValid@data[[var]], na.rm = T)
...
end_time <- Sys.time()
timeforlastloop <- end_time-start_time


# Estimation of the total execution time ----------------------------------

execution_time_explorative_statistic <- timeforsummarygraphics     # execution time for exploratory data analysis code
execution_time_explorative_statistic <- round(execution_time_explorative_statistic,digits=2)

execution_time_last_loop <- timeforlastloop                        # execution time for last loop [for (l in 1:nrofcombinations)]
execution_time_last_loop <- round(execution_time_last_loop,digits=2)
 

averagetime_one_run <- execution_time_last_loop/nrun
averagetime_one_run <- round(averagetime_one_run,digits=2)

estimatedtotaltime_min <- execution_time_last_loop*no.combi*3
estimatedtotaltime_min <- round(estimatedtotaltime_min,digits=2)
estimatedtotaltime_hours <- estimatedtotaltime/60
estimatedtotaltime_hours <- round(estimatedtotaltime_hours,digits=2)
estimatedtotaltime_days <- estimatedtotaltime_hours/24
estimatedtotaltime_days <- round(estimatedtotaltime_days,digits=2)

setwd(script_dir)
fileConn<-file("estimated_execution_time.txt")
writeLines(c(paste0("The execution time for exploratory data analysis: ",execution_time_explorative_statistic, " minutes"),
             paste0("The estimated average time of one run program run is approx.: ",averagetime_one_run," minutes"),
             paste0("The estimated total execution time is: ",estimatedtotaltime_min, " minutes"),
             paste0("The estimated total execution time is: ",estimatedtotaltime_hours, " hours"),
             paste0("The estimated total execution time is: ",estimatedtotaltime_days, " days"),
             paste0("Number of program runs: ",nrun)), fileConn)
close(fileConn)
